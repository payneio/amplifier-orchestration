...
class MultiContextOrchestrator:
    ...
    async def _get_or_create_session(self, profile: str) -> AmplifierSession:
        ...
        if profile not in self.sessions:
            logger.debug(f"Creating new session for profile: {profile}")
            logger.debug(f"[TRACE] Loading mount plan for profile: {profile}")
            config = self._load_mount_plan(profile)
            logger.debug("[TRACE] Mount plan loaded, creating AmplifierSession")
            session = AmplifierSession(config, loader=self.loader)
            logger.debug("[TRACE] Calling session.__aenter__() to initialize")
            await session.__aenter__()  # Initialize
            logger.debug(f"[TRACE] Session initialized for profile: {profile}")
            self.sessions[profile] = session
        else:
            logger.debug(f"[TRACE] Reusing existing session for profile: {profile}")
    ...
    def get_or_create_context(self, context_name: str) -> ExecutionContext:
        ...
        if context_name not in self.contexts:
            self.contexts[context_name] = ExecutionContext(name=context_name, max_history=self.max_context_history)
            logger.debug(f"Created new context: {context_name}")

        return self.contexts[context_name]

    async def execute_task(self, task_dict: dict[str, Any], default_profile: str | None = None) -> dict[str, Any]:
        """
        ...
        logger.debug(f"[TRACE] execute_task START: context={context_name}, profile={profile}")
        ...
        logger.debug(f"Executing task in context '{context_name}' with profile '{profile}'")
        print(f"  🔄 [{context_name}] Starting task with profile '{profile}'...")

        # Get or create context for tracking
        logger.debug(f"[TRACE] Getting context for '{context_name}'")
        context = self.get_or_create_context(context_name)
        context.add_message("user", prompt)
        logger.debug("[TRACE] Context retrieved, adding user message")

        try:
            # Get session for this profile
            logger.debug(f"[TRACE] Getting session for profile '{profile}'")
            session = await self._get_or_create_session(profile)
            logger.debug("[TRACE] Session obtained, calling session.execute()")

            # Execute via Amplifier Core
            # Note: session.execute() maintains its own history internally
            response = await session.execute(prompt)
            logger.debug(f"[TRACE] session.execute() returned: {len(response)} chars")

            # Track in context for metadata
            context.add_message("assistant", response)

            logger.debug(f"Task completed in context '{context_name}'")
            logger.debug(f"[TRACE] execute_task SUCCESS: context={context_name}")
    ...
    async def execute_phase(
        ...
        logger.debug(f"Starting phase '{phase_name}' with {len(tasks)} tasks ({execution_mode})")
        print(f"\n{'=' * 70}")
        print(f"📋 PHASE: {phase_name}")
        print(f"   Mode: {execution_mode.upper()}")
        print(f"   Tasks: {len(tasks)}")
        print(f"{'=' * 70}\n")
        logger.debug(f"[TRACE] execute_phase START: phase={phase_name}, mode={execution_mode}, tasks={len(tasks)}")

        if execution_mode == "parallel":
            logger.debug(f"[TRACE] Creating {len(tasks)} parallel task coroutines")
            # Execute all tasks concurrently
            task_coroutines = [self.execute_task(task, default_profile) for task in tasks]
            logger.debug(f"[TRACE] Calling asyncio.gather() with {len(task_coroutines)} coroutines")
            results = await asyncio.gather(*task_coroutines, return_exceptions=True)
            logger.debug(f"[TRACE] asyncio.gather() returned {len(results)} results")
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    logger.debug(f"[TRACE] Task {i} completed successfully")
                    processed_results.append(result)

            phase_elapsed = time.time() - phase_start
            logger.debug(f"[TRACE] execute_phase COMPLETE: phase={phase_name}")
            print(f"\n  ⏱️  Phase '{phase_name}' completed in {phase_elapsed:.1f}s")
            return processed_results

        # sequential
        logger.debug(f"[TRACE] Sequential execution of {len(tasks)} tasks")
        results = []
        for i, task in enumerate(tasks):
            logger.debug(f"[TRACE] Starting sequential task {i + 1}/{len(tasks)}")
            result = await self.execute_task(task, default_profile)
            results.append(result)
            logger.debug(f"[TRACE] Sequential task {i + 1}/{len(tasks)} completed")

            # Stop on failure if configured (future enhancement)
            # For MVP, we continue even on failure
            if not result["success"]:
                logger.warning(f"Task failed in context '{result['context_name']}', continuing...")
    ...
    async def execute_workflow(self, workflow: Workflow) -> dict[str, Any]:
        ...
        Returns:
            Dictionary with workflow execution results:
                ...
                - failed_tasks: int
        """
        logger.debug(f"Starting workflow: {workflow.name}")
        logger.debug(f"[TRACE] execute_workflow START: {workflow.name}, phases={len(workflow.phases)}")
        ...
        for i, phase in enumerate(workflow.phases):
            logger.debug(f"[TRACE] Starting phase {i + 1}/{len(workflow.phases)}: {phase.name}")
            phase_dict = phase.model_dump()
            logger.debug(f"[TRACE] Calling execute_phase for '{phase.name}'")
            phase_results = await self.execute_phase(phase_dict, workflow.default_profile)
            logger.debug(f"[TRACE] Phase {i + 1}/{len(workflow.phases)} returned {len(phase_results)} results")

            all_phase_results.append({"phase_name": phase.name, "results": phase_results})
        ...
        logger.debug(f"Workflow '{workflow.name}' completed: {successful_tasks}/{total_tasks} tasks successful")
        logger.debug(f"[TRACE] execute_workflow COMPLETE: {workflow.name}")
    ...
    async def cleanup(self):
        ...
        logger.debug(f"Cleaning up {len(self.sessions)} sessions")

        for profile, session in self.sessions.items():
            try:
                await session.__aexit__(None, None, None)
                logger.debug(f"Closed session for profile: {profile}")
            except Exception as e:
