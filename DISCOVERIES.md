...
## Tool-Filesystem Ignoring working_dir Config (2025-12-12)

### Issue

Sessions with `amplified_dir` set would fail when using relative file paths with tool-filesystem (read_file, write_file, edit_file). For example:
- Session has `amplified_dir="projects/cstf/projects/civil-dynamics"`
- File exists at `/data/projects/cstf/projects/civil-dynamics/OVERVIEW.md`
- Calling `read_file(file_path="OVERVIEW.md")` fails with "Path not found: OVERVIEW.md"

### Root Cause

The tool-filesystem module receives `working_dir` in its config (correctly set by amplifierd) but never uses it to resolve relative paths. The tools just do:

```python
path = Path(file_path)  # Resolves against daemon CWD, not working_dir
```

This affects all three tools: read.py, write.py, and edit.py.

**Important distinction:** `working_dir` is a CONFIG parameter (set at tool initialization), not a CALL parameter (passed at runtime). The config was correct, but the tools weren't using it.

### Solution

Updated all three tools in tool-filesystem to resolve relative paths against the configured `working_dir`:
...
**In `execute` method (after @mention handling):**
```python
else:
    path = Path(file_path)
...
1. Source: `registry/tools/tool-filesystem/amplifier_module_tool_filesystem/{read,write,edit}.py`
2. Cached: `.amplifierd/share/profiles/*/behaviors/filesystem/tools/tool-filesystem/amplifier_module_tool_filesystem/{read,write,edit}.py`

### Key Learnings

1. **Config vs call parameters**: Tool configuration (like working_dir) is set once at initialization, not passed with each call
2. **amplifierd was correct**: The daemon properly configures working_dir in mount_plan.json - the bug was in tool implementation
3. **Fix in multiple locations**: Need to fix both registry source (for upstream) and cached profiles (for immediate use)
4. **Test with real scenarios**: The bug only manifests when actually using sessions with amplified_dir and relative paths

### Prevention

- When adding new path-based tools, always resolve relative paths against `working_dir` config
- Check that tool-bash, tool-search, and other path-aware tools also respect working_dir
- Add integration tests for sessions with amplified_dir using relative paths
- Document path resolution behavior in tool descriptions

### Impact

**Before fix:**
- Relative paths in amplified sessions didn't work
- Users had to use absolute paths (poor UX)
- Defeated the purpose of having working_dir config

**After fix:**
- Relative paths resolve correctly against session's amplified_dir
- Natural workflow: create session in project → use relative paths
- @mention paths continue to work as before
...
## Auto-Start Sessions (2025-01-12)

### Issue

Sessions required a two-step process: create (CREATED state) → start (ACTIVE state) → use. This created unnecessary UX friction requiring 3 clicks before users could send messages.

### Root Cause

The two-state design (CREATED vs ACTIVE) was architectural over-engineering:

1. **No functional separation**: Nothing happened between CREATED and ACTIVE states
2. **No enforcement**: Code never validated ACTIVE status before allowing messages
3. **Future-proofing**: Built for hypothetical pre-start initialization that was never implemented
4. **UX friction**: Users had to explicitly start sessions even though they were immediately usable

### Solution

Eliminated the CREATED state transition by auto-starting sessions on creation:

**Backend changes** (`amplifier_library/sessions/manager.py`):
```python
# Sessions now created in ACTIVE state
metadata = SessionMetadata(
    status=SessionStatus.ACTIVE,  # Changed from CREATED
    started_at=now,  # Set immediately
    ...
)

# start_session() made idempotent for backwards compatibility
def start_session(self, session_id: str) -> None:
    """No-op for ACTIVE sessions, handles legacy CREATED sessions."""
    if session.status == SessionStatus.ACTIVE:
        return  # Idempotent no-op
    # ... handle legacy CREATED sessions
```

**Frontend simplification** (`webapp/src/features/directories/components/SessionsList.tsx`):
```tsx
// Before: Create → Start → Navigate (3 API calls, complex state)
const handleCreateSession = async () => {
  const session = await createSession.mutateAsync(...);
  await startSession.mutateAsync(session.sessionId);
  navigate(...);
};

// After: Create → Navigate (1 API call, simple)
const handleCreateSession = async () => {
  const session = await createSession.mutateAsync(...);
  navigate(...);
};
...
- CREATED state kept in enum for old session files
- `start_session` endpoint preserved as idempotent no-op
- Legacy CREATED sessions can still be started
- No breaking API changes
- Zero migration issues

### Key Learnings

1. **Question state transitions**: Not all state machines need multiple states
2. **Validate design intent**: If code doesn't enforce a state, why have it?
3. **Ruthless simplicity wins**: Removing the transition simplified both frontend and backend
4. **Idempotency enables change**: Making operations idempotent allows safe default changes
5. **YAGNI principle**: Future-proofing for unused features adds complexity without value

### Impact

- **UX**: 3-click flow → 1-click flow (67% reduction in user actions)
- **Frontend**: Removed mutation, simplified state management, cleaner code
- **Backend**: Eliminated unnecessary state transition, clearer lifecycle
- **Tests**: 583 tests passing, comprehensive coverage maintained
- **Philosophy**: Textbook example of "ruthless simplicity" in practice

### Prevention

- Challenge any state transition that happens instantly
- Validate that state machines enforce meaningful invariants
- Remove states that exist "just in case" for hypothetical futures
- Test if removing a feature breaks anything before adding complexity
- Apply "ruthless simplicity" philosophy to architectural decisions
